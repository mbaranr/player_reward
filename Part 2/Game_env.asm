...
x_e:		.word	0		# x and
y_e:		.word	0		# y coordinates for enemy

x_t:		.word 	0		# x and
y_t:		.word	0		# y coordinates of enemy target cell 
...
clear_screen:	.asciiz "\f"
	...
	.globl	init_env, rows, columns, x_p, y_p, x_r, y_r, delete_curr_pos, player, reward, score_track, score_amount, clear_screen, score, enemy, x_e, y_e, x_t, y_t, ws, AGGRO, aggro
...
init_enemy:
	...
	# generate initial random pos for enemy
	...
	jal	rand_num_enemy		# generating random enemy position
	
	jal	enemy			# displaying enemy
...
enemy:
	...
	# display given enemy pos

	addi	$sp, $sp, -4		# reserve space on the stack
	sw	$ra, ($sp)		# store the return address of nested call
	
	la	$a2, e			# load the address of "E" character
	
	la	$a0, x_e		# getting
	la	$a1, y_e		# x and y
	lw	$a0, ($a0)		# pos
	lw	$a1, ($a1)		# of enemy | setting cursor to (x_e, y_e)
	
	jal 	display			# displaying enemy
...
aggro:
	...
	# printing the message "AGGRO!" if the enemy will attack in the next turn

	addi	$sp, $sp, -4		# reserve space in the stack
	sw	$ra, ($sp)		# store the return address
	
	la	$a2, AGGRO		# loading address of "AGGRO!" message
	
	li	$a0, 13			# setting cursor to
	li	$a1, 0			# (13,0)
	
	jal 	display			# display the "AGGRO!" alert
